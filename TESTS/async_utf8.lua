...
if vim.fn.executable("rg") == 1 then
  ...
  check(
    "async: found 3 occurrences",
    got and got.items and #got.items == 3,
    got and got.items and #got.items
  )
else
...
do
  ...
  check(
    "utf8: both replaced (spots==2, skipped==0)",
    spots == 2 and skipped == 0,
    string.format("spots=%d skipped=%d", spots, skipped)
  )
  check(
    "utf8: result correct + emoji intact",
    new_lines[1] == "Grüße Mueller 😀 Mueller",
    new_lines[1]
  )
  ...
  check(
    "utf8: file written, umlaut+emoji preserved",
