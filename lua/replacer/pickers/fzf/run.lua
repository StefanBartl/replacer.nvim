...
--- fzf-lua picker wiring: build source, provide preview, and wire actions that call apply_func.
...
function M.run(items, new_text, cfg, apply_func)
  ...
  -- default actions: replace selected, and ctrl-a => apply all (with confirm if configured)
  ...
  -- Build opts, preserving user's cfg.fzf.actions if present (user can override)
  local base_opts = {
  ...
  -- Merge user-provided fzf options; user-provided actions override defaults
  local opts = vim.tbl_deep_extend("force", base_opts, cfg.fzf or {})

  -- Launch fzf-lua with the computed options and source
